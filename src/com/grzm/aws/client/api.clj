...
(ns com.grzm.aws.client.api
  "API functions for using a client to interact with AWS services."
  (:require [clojure.core.async :as a]
            [clojure.string :as str]
            [clojure.tools.logging :as log]
            [com.grzm.aws.client :as client]
            [com.grzm.aws.client.api.async :as api.async]
            [com.grzm.aws.client.shared :as shared]
            [com.grzm.aws.dynaload :as dynaload]
            ...
            [com.grzm.aws.signers] ;; implements multimethods
            ))
...
(defn client
  ...
  [{:keys [api region region-provider retriable? backoff credentials-provider endpoint-override _http-client]
  ...
  (let [service              (service/service-description (name api))
        ...
        endpoint-provider    (endpoint/default-endpoint-provider
                              ...
                              endpoint-override)]
    (dynaload/load-ns (symbol (str "com.grzm.aws.protocols." (get-in service [:metadata :protocol]))))
    (client/->Client
                                             ...
                                             (let [i (client/-get-info c)]
                                               (-> i
                                                   ...
                                                   (assoc :region (-> i :region-provider region/fetch)
                                                          ;; XXX endpoint/fetch takes 2 args.
